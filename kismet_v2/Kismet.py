...
class KismetModule():
    def __init__(self,module_file,tracery_files=[],
        ...
        with open(f'{self.module_file}_rules.swi', 'w') as asp_file:
    ...
    def population2asp(self):
        with open(f'{self.module_file}_population.swi','w') as population:
            for name in self.population:
    ...
    def actions2asp(self,actions):
        with open(f'{self.module_file}_actions.swi','w') as action_file:
            for action in actions:
                action_file.write(f'occurred(action({",".join(action)})).\n')

                     
    def calculate_volitions(self):
        volitions = solve(['default.swi', f'{self.module_file}_rules.swi', f'{self.module_file}_population.swi', 'testing.swi','volition.lp','-t','8'])
        return volitions
    def calculate_action_results(self):
        action_results = solve(['default.swi', f'{self.module_file}_rules.swi', f'{self.module_file}_population.swi', f'{self.module_file}_actions.swi', 'testing.swi','results_processing.lp','-t','8'])
        return action_results
    
    def calculate_observability(self):
        visibility_results = solve(['default.swi', f'{self.module_file}_rules.swi', f'{self.module_file}_population.swi', f'{self.module_file}_actions.swi', 'testing.swi','observation.lp','-t','8'])
        return visibility_results
    
    def knowledge2asp(self):        
        with open(f'{self.module_file}_history.swi','w') as history_file:
